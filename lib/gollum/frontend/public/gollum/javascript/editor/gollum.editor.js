...
(function($) {
  ...
  $.GollumEditor = function( IncomingOptions ) {
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      if (ActiveOptions.EditorMode == 'code') {
        $('#gollum-editor-body').hide();
        $('#gollum-editor-body').before("<div id='gollum-editor-ace'></div>");

        editor = ace.edit("gollum-editor-ace");
        ...
        editor.getSession().setValue($('#gollum-editor-body').val());


        $.GollumEditor.editor = editor;

        $.GollumEditor.setSyntaxMode();

        $.GollumEditor.editor.getSession().on('change', $.GollumEditor.updateSource);
        $('#wiki_format').change($.GollumEditor.setSyntaxMode);
      }
      ...
      if ( EditorHas.editSummaryMarkup() ) {
        $.GollumEditor.Placeholder.add($('#gollum-editor-edit-summary #gollum-editor-message-field'));
        $.GollumEditor.Placeholder.add($('#gollum-editor-edit-summary #gollum-editor-github-email-field'));
  ...
  /**
   *  $.GollumEditor.updateSource
   *  Updates between gollum-editor-ace and #gollum-editor-body
   */
  $.GollumEditor.updateSource = function(event) {
    var newValue = $.GollumEditor.editor.getSession().getValue();
    $('#gollum-editor-body').each(function(_,item) { item.value = newValue });
  };
   ...
   *  Updates between gollum-editor-ace and #gollum-editor-body
   */
  $.GollumEditor.setSyntaxMode = function(event) {
    var format  = $('#wiki_format').val()
    var newMode = "ace/mode/" + format

    require([newMode], function(mode) {
      if (mode) {
        $.GollumEditor.editor.getSession().setMode(new mode.Mode());
      } else {
        var textMode = require('ace/mode/text');
        $.GollumEditor.editor.getSession().setMode(new textMode.Mode());
      }
    });
  };
